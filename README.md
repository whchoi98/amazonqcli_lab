# Kiro CLI Lab 환경 구축 가이드

Kiro CLI와 MCP(Model Context Protocol) 서버를 활용한 AWS 인프라 실습 환경을 구축하기 위한 자동화 스크립트와 CloudFormation 템플릿 모음입니다.
...
이 프로젝트는 AWS 클라우드 환경에서 Kiro CLI를 활용한 실습 환경을 자동으로 구축합니다. 네트워킹 인프라부터 개발 도구, AI 지원 도구까지 포괄적인 환경을 제공합니다.
...
- **Kiro CLI**: Kiro CLI 로그인 후 /mcp 명령으로 도움말 확인
