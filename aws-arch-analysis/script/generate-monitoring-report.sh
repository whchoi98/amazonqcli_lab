...
# 서브넷 데이터 분석
if [ -f "networking_subnets.json" ] && [ -s "networking_subnets.json" ]; then
    SUBNET_COUNT=$(jq '.rows | length' networking_subnets.json)
    PUBLIC_SUBNET_COUNT=$(jq '[.rows[] | select(.map_public_ip_on_launch == true)] | length' networking_subnets.json)
    PRIVATE_SUBNET_COUNT=$(jq '[.rows[] | select(.map_public_ip_on_launch == false)] | length' networking_subnets.json)
    
    echo "**총 서브넷 수:** ${SUBNET_COUNT}개 (퍼블릭: ${PUBLIC_SUBNET_COUNT}개, 프라이빗: ${PRIVATE_SUBNET_COUNT}개)" >> 02-networking-analysis.md
    echo "" >> 02-networking-analysis.md
    echo "| 서브넷 ID | VPC ID | CIDR Block | AZ | 타입 | 사용 가능 IP |" >> 02-networking-analysis.md
    echo "|-----------|--------|------------|----|----- |-------------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.subnet_id) | \(.vpc_id) | \(.cidr_block) | \(.availability_zone) | \(if .map_public_ip_on_launch then "퍼블릭" else "프라이빗" end) | \(.available_ip_address_count) |"' networking_subnets.json >> 02-networking-analysis.md
    
    # VPC별 서브넷 분포
    echo "" >> 02-networking-analysis.md
    echo "### VPC별 서브넷 분포" >> 02-networking-analysis.md
    echo "| VPC ID | 총 서브넷 | 퍼블릭 | 프라이빗 |" >> 02-networking-analysis.md
    echo "|--------|-----------|--------|----------|" >> 02-networking-analysis.md
    jq -r '.rows | group_by(.vpc_id) | .[] | "| \(.[0].vpc_id) | \(length) | \([.[] | select(.map_public_ip_on_launch == true)] | length) | \([.[] | select(.map_public_ip_on_launch == false)] | length) |"' networking_subnets.json >> 02-networking-analysis.md
else
    echo "서브넷 데이터를 찾을 수 없습니다." >> 02-networking-analysis.md
fi
...
if [ -f "security_groups.json" ] && [ -s "security_groups.json" ]; then
    ...
    echo "|---------|--------|--------|------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.group_id) | \(.group_name // "N/A") | \(.vpc_id) | \(.description // "설명 없음") |"' security_groups.json >> 02-networking-analysis.md
...
if [ -f "networking_route_tables.json" ] && [ -s "networking_route_tables.json" ]; then
    ...
    echo "|------------------|--------|-------------|------------------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.route_table_id) | \(.vpc_id) | \(.main) | \(.associations | length) |"' networking_route_tables.json >> 02-networking-analysis.md
else
...
# VPC 엔드포인트 데이터 분석
if [ -f "networking_vpc_endpoints.json" ] && [ -s "networking_vpc_endpoints.json" ]; then
    VPC_ENDPOINT_COUNT=$(jq '.rows | length' networking_vpc_endpoints.json)
    GATEWAY_ENDPOINT_COUNT=$(jq '[.rows[] | select(.vpc_endpoint_type == "Gateway")] | length' networking_vpc_endpoints.json)
    INTERFACE_ENDPOINT_COUNT=$(jq '[.rows[] | select(.vpc_endpoint_type == "Interface")] | length' networking_vpc_endpoints.json)
    
    echo "**총 VPC 엔드포인트:** ${VPC_ENDPOINT_COUNT}개 (Gateway: ${GATEWAY_ENDPOINT_COUNT}개, Interface: ${INTERFACE_ENDPOINT_COUNT}개)" >> 02-networking-analysis.md
    echo "" >> 02-networking-analysis.md
    echo "| 엔드포인트 ID | 서비스명 | 타입 | VPC ID | 상태 |" >> 02-networking-analysis.md
    echo "|---------------|----------|------|--------|------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.vpc_endpoint_id) | \(.service_name) | \(.vpc_endpoint_type) | \(.vpc_id) | \(.state) |"' networking_vpc_endpoints.json >> 02-networking-analysis.md
else
    echo "VPC 엔드포인트 데이터를 찾을 수 없습니다." >> 02-networking-analysis.md
fi
...
# NAT 게이트웨이 데이터 분석
if [ -f "networking_nat.json" ] && [ -s "networking_nat.json" ]; then
    NAT_COUNT=$(jq '.rows | length' networking_nat.json)
    echo "**총 NAT 게이트웨이:** ${NAT_COUNT}개" >> 02-networking-analysis.md
    echo "" >> 02-networking-analysis.md
    echo "| NAT 게이트웨이 ID | 서브넷 ID | 상태 | 연결성 타입 | Elastic IP |" >> 02-networking-analysis.md
    echo "|-------------------|-----------|------|-------------|------------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.nat_gateway_id) | \(.subnet_id) | \(.state) | \(.connectivity_type // "public") | \(.nat_gateway_addresses[0].public_ip // "N/A") |"' networking_nat.json >> 02-networking-analysis.md
else
    echo "NAT 게이트웨이 데이터를 찾을 수 없습니다." >> 02-networking-analysis.md
fi
...
if [ -f "networking_acl.json" ] && [ -s "networking_acl.json" ]; then
    ...
    echo "|--------|--------|----------|------------------|" >> 02-networking-analysis.md
    jq -r '.rows[] | "| \(.network_acl_id) | \(.vpc_id) | \(.is_default) | \(.associations | length) |"' networking_acl.json >> 02-networking-analysis.md
else
